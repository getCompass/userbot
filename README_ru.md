...
Текст сообщения с упоминанием по бейджу должен иметь вид:</br>
`["@"|0|"all"]` – чтобы упомянуть всех пользователей чата;</br>
...
## Postman: готовая коллекция и настройки окружения
...
Важно: в коллекции есть GET-запросы без параметров и тела.
Они используются только как заголовки-разделители с описанием следующего запроса.
Сразу под таким заголовком находится рабочий эндпоинт, который выполняет соответствующее действие.

[Посмотреть коллекцию](postman/compass_userbot_api_ru.postman_collection.json)
...
[Посмотреть окружение SaaS](postman/saas_environment.postman_environment.json)

**Окружение для On-Premise установки**  
Переменные:  
- `api_base_url` — ваш домен, например `https://<yourdomain>/userbot/`  
- `bot_token`    — токен вашего бота  

[Посмотреть окружение On-Premise](postman/onpremise_environment.postman_environment.json)
